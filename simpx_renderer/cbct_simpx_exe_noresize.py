...
def process_one(
    ...
    # Resize disabled: raw pano width follows the number/order of rendered rays.
    ...
    # Resize disabled: save raw renderer resolution.
    ...
    saved_size = img.size
    expected_size = (int(pano_raw.shape[1]), int(pano_raw.shape[0]))
    if saved_size != expected_size:
        raise RuntimeError(
            f"Saved pano size mismatch: got PIL size={saved_size}, expected={expected_size}"
        )
    ...
    meta = (
        ...
        f"raw_pano_shape_pass1_H_W: {raw_shape_1}\n"
        f"raw_pano_shape_pass2_H_W: {raw_shape_2}\n"
        f"final_pano_shape_H_W: {tuple(pano_raw.shape)}\n"
        f"final_png_size_W_H: {saved_size}\n"
        f"resize: disabled\n"
        f"deprecated_pano_h_arg: {pano_h}\n"
        f"deprecated_pano_w_arg: {pano_w}\n"
        f"clip_max: {clip_max}\n"
        f"q_percentile: {q}\n"
    ...
    print(
        ...
        f"raw_shape={raw_shape_2} -> final_HxW={tuple(pano_raw.shape)} | {out_png}"
    )
...
def main():
    ...
    ap.add_argument("--geometry_root", default="/home/alphayoung8/log1/nebla/simpx_renderer/center_geometries")
    ap.add_argument("--out_dir", default="/home/alphayoung8/log1/nebla/simpx_renderer/simpx_result_noresize")
    ...
    ap.add_argument("--pano_h", type=int, default=200, help="Deprecated: ignored when resize is disabled.")
    ap.add_argument("--pano_w", type=int, default=350, help="Deprecated: ignored when resize is disabled.")
    ...
    with log_path.open("w", encoding="utf-8") as f:
        ...
        f.write("resize=disabled; final_pano_H_W follows raw renderer output\n")
        f.write(f"deprecated_target_pano_H_W_args=({args.pano_h}, {args.pano_w})\n")
