...
def _make_center_region_runs(ray_center_ids_np: np.ndarray):
    """
    최종 pano의 column 순서와 동일한 ray_center_ids 배열에서
    연속된 center id 구간(run)을 계산합니다.

    return:
        list of dicts with keys:
            cid, raw_start, raw_end, ray_count
    """
    ids = np.asarray(ray_center_ids_np, dtype=np.int32).reshape(-1)
    if ids.size == 0:
        return []
    ...
    runs.append({
        "cid": prev,
        "raw_start": int(start),
        "raw_end": int(ids.size),
        "ray_count": int(ids.size - start),
    })

    return runs


def save_pano_center_regions_debug(
    pano_rgba: np.ndarray,
    ray_center_ids_np: np.ndarray,
    save_path: Path,
    meta_path: Path = None,
):
    """
    최종 저장되는 pano_final.png와 같은 크기의 이미지 위에
    각 horizontal region이 어느 rotation center(C0~C20)의 ray들로부터
    만들어졌는지 표시합니다.

    주의:
      - pano_rgba는 이미 resize_pano_raw_np() 이후의 최종 RGBA 이미지입니다.
      - ray_center_ids_np는 resize 이전 raw ray column 순서입니다.
      - 따라서 raw column index를 최종 width로 선형 대응시켜 영역을 그립니다.
      - 표시 순서는 rotation_geometry npz에 저장된 ray order를 그대로 따릅니다.
    """
    arr = np.asarray(pano_rgba)
    if arr.ndim != 3 or arr.shape[2] not in (3, 4):
        raise ValueError(f"pano_rgba must have shape (H,W,3/4), got {arr.shape}")
    ...
    ids = np.asarray(ray_center_ids_np, dtype=np.int32).reshape(-1)
    if ids.size == 0:
        raise ValueError("ray_center_ids_np is empty")

    base = Image.fromarray(arr.astype(np.uint8), mode="RGBA")
    W, H = base.size
    R = int(ids.size)

    runs = _make_center_region_runs(ids)

    # C label을 보기 쉽게 하기 위한 반복 palette입니다.
    palette = [
        (255, 64, 64),
        (64, 160, 255),
        (64, 220, 120),
    ...
    top_h = max(18, min(34, H // 8))
    meta_lines = [
        "# Center-region debug for final resized pano",
        "# left-to-right order follows ray_center_ids in rotation_geometry npz",
        f"final_pano_size_W_H: ({W}, {H})",
        f"raw_ray_count: {R}",
        "columns: cid, raw_start, raw_end_exclusive, ray_count, resized_left, resized_right",
    ]
    ...
    for run in runs:
        ...
        # align_corners=False resize의 column 대응을 근사적으로 표시합니다.
        left = int(np.floor(raw_start * W / R))
        right = int(np.ceil(raw_end * W / R) - 1)
        ...
        # 전체 영역은 아주 약하게 tint, 상단 bar는 진하게 표시합니다.
        draw.rectangle((left, 0, right, H - 1), fill=color + (28,))
        draw.rectangle((left, 0, right, top_h), fill=color + (120,))
        draw.line((left, 0, left, H - 1), fill=color + (230,), width=2)
        draw.line((right, 0, right, H - 1), fill=color + (160,), width=1)

        label = f"{cid}"
        tx = (left + right) // 2 - 5 * len(label) // 2
        tx = int(np.clip(tx, 0, max(0, W - 1)))
        ty = 3

        # text shadow + white label
        draw.text((tx + 1, ty + 1), label, fill=(0, 0, 0, 220))
        draw.text((tx, ty), label, fill=(255, 255, 255, 255))

        meta_lines.append(
            f"C{cid}\t{raw_start}\t{raw_end}\t{ray_count}\t{left}\t{right}"
        )

    # 외곽선
    draw.rectangle((0, 0, W - 1, H - 1), outline=(255, 255, 255, 180), width=1)

    debug_img = Image.alpha_composite(base, overlay)
    save_path.parent.mkdir(parents=True, exist_ok=True)
    debug_img.save(str(save_path))
...
def process_one(
    ...
    # 최종 pano의 horizontal region을 center별로 표시하기 위해
    # raw ray column마다 어떤 center에서 나온 ray인지 읽어둡니다.
    with np.load(str(geom_path), allow_pickle=False) as geom_npz:
        ray_center_ids_np = geom_npz["ray_center_ids"].astype(np.int32)

    if int(ray_center_ids_np.shape[0]) != int(ray_origins.shape[0]):
        raise RuntimeError(
            f"ray_center_ids length mismatch: "
            f"ids={ray_center_ids_np.shape[0]}, rays={int(ray_origins.shape[0])}"
        )
    ...
    center_region_debug_path = out_dir / "debug_pano_center_regions.png"
    center_region_meta_path = out_dir / "debug_pano_center_regions_meta.txt"
    save_pano_center_regions_debug(
        pano_rgba=pano_rgba,
        ray_center_ids_np=ray_center_ids_np,
        save_path=center_region_debug_path,
        meta_path=center_region_meta_path,
    )
    ...
    meta = (
        ...
        f"center_region_debug_png: {center_region_debug_path}\n"
        f"center_region_debug_meta: {center_region_meta_path}\n"
        f"clip_max: {clip_max}\n"
        f"q_percentile: {q}\n"
